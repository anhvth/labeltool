...
from labelme.model_line import imgpath2lines
...
class MainWindow(QtWidgets.QMainWindow, WindowMixin):
    ...
    def __init__(self, config=None, filename=None, output=None):
        ...
        self.dirty = False
        self.imgpath2lines = imgpath2lines
        ...
        createMode = action(
            'Perform model-border',
            lambda: self.detectLine(),
            shortcuts['create_line'],
            'objects',
            'Start detecting line with anson-model',
            enabled=True,
        )
    ...
    def detectLine(self):
        d = self.imgpath2lines(self.imagePath)
        img_path = self.imagePath
        json_path = img_path.replace('.png', '.json')
        with open(json_path, 'w') as f:
            json.dump(d, f)
        print('Dump at:', json_path)
